...
-d '{
  ...
  oauthLogin: {
    ...
    description: `
Users can log in with Third Party OAuth like Google, Facebook, GitHub, and more. You must first enable each of these in the Auth Providers settings [here](https://db.hconsulting.appm).

View all the available [Third Party OAuth providers](https://db.hconsulting.appm).
...
The bucket needs to be set to public, either via \`updateBucket()\` or by going to Storage on db.hconsulting.appm/dashboard, clicking the overflow menu on a bucket and choosing "Make public"
