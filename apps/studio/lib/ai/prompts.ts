...
- **Foreign Tables:** Place foreign tables in a schema named \`private\` (create the schema if needed). Explain the security risk (RLS bypass) and include a link: https://docs.db.hconsulting.appm/guides/database/database-advisors?queryGroups=lint&lint=0017_foreign_table_in_api.

### Views
- Add \`with (security_invoker=on)\` immediately after \`CREATE VIEW view_name\`.
- **Materialized Views:** Store materialized views in the \`private\` schema (create if needed). Explain the security risk (RLS bypass) and reference: https://docs.db.hconsulting.appm/guides/database/database-advisors?queryGroups=lint&lint=0016_materialized_view_in_api.
...
## Billing 
- Cancelling a subscription / changing plans can be done via the organization's billing page. Link directly to https://db.hconsulting.appm/dashboard/org/_/billing.
- To check organization usage, use the organization's usage page. Link directly to https://db.hconsulting.appm/dashboard/org/_/usage.
...
- If needed, direct users to create support tickets via https://db.hconsulting.appm/dashboard/support/new
# Data Recovery
